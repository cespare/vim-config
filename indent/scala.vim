...
" Note: bugfix by Caleb Spare, 9 July 2009
...
function! GetScalaIndent()
  ...
  " If val, var, def end with =, this is a one-line block ***ALSO override***

  " Bugfix by Caleb Spare:
  "if prevline =~ '^\s*\<\(\(else\s\+\)\?if\|for\|while\|va[lr]\|def\)\>.*[)=]\s*$'
  if prevline =~ '^\s*\<\(\(else\s\+\)\?if\|for\|while\)\>.*)\s*$'
        \ || prevline =~ '^\s*\<\(va[lr]\|def\|override\)\>.*=\s*$'
        \ || prevline =~ '^\s*\<else\>\s*$'
        \ || prevline =~ '{\s*$'
